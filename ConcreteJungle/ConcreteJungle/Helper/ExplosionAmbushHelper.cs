...
namespace ConcreteJungle.Helper
{
    public static class ExplosionAmbushHelper
    {
        public static void SpawnAmbush(Vector3 originPos)
        {
			...
			Mod.Log.Info?.Write($"Explosion ambush will apply {numBlasts} blasts.");
			...
			Mod.Log.Debug?.Write($"Found origin hex: {originHex} with {adjacentHexes.Count} adjacent hexes within {2} hexes");
			for (int i = 0; i < numBlasts - 1; i++) 
			{
				...
				Mod.Log.Debug?.Write($" Adding additional blast position: {newHexPos}");
				...
				Mod.Log.Debug?.Write($" TerrainCell cached height: {melDataCell.relatedTerrainCell.cachedHeight}");
			...
			Mod.Log.Debug?.Write($"Sending AddSequence message for ambush explosion with {blastsForAttack.Count} weapons and {blastPositions.Count} blasts");
			try
			{
